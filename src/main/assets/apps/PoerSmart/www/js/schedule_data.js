...
function addToData(index, programTime, nextProgramTime) {
    //	if (addMarkPoint) {
    //		addMarkPoint = false;
    var tempTS = +new Date(1970, 1, 1, 0, 0, 0);
    var tempTime = new Date(+tempTS + programTime.Sec * 1000);
    var now = new Date();
    var markTime = new Date(now.getFullYear(), now.getMonth() + 1, now.getDate(), tempTime.getHours(), tempTime.getMinutes());
    var temprature = roundUp(programTime.Temprature / 10 * 2, 0) / 2;
    var itemColor = getColor(temprature);
    markPointData.push({
        index: index,
        name: timeList[markPointIndex],
        coord : [dateToMinStr(markTime), temprature],
        value: temprature,
        xAxis: markPointIndex,
        yAxis: temprature,
        ...
        xAxis: {
            ...
            axisLabel : {
                formatter : function (param) {
                    ...
                    // return ultZeroize(time.getHours() , 2) + ":" + ultZeroize(time.getMinutes(), 2);
                    return ultZeroize(time.getHours() , 2);
                }
            }
        },
        yAxis: {
